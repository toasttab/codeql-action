...
async function getTrapHash(language, config, logger) {
    ...
    if (trapRootFiles.length === 0) {
        throw new Error(`Trap directory ${trapDir} is empty. Has the 'create-database' action been used with 'keep-trap: true'?`);
    }
    logger.info(`trapDir ${trapDir} content:`);
    logger.info(cp.execFileSync("ls", [trapDir]).toString());
    let exec = util_1.promisify(cp.exec);
    let hashResponse = await exec("find . -mindepth 2 -type f -name \\*.trap.gz -print0 | sort -z | xargs -0 zcat | grep -v '^extraction_time' | shasum -", { cwd: trapDir });
