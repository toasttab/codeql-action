...
async function runAnalyze(repositoryNwo, commitOid, ref, analysisKey, analysisName, workflowRunID, checkoutPath, environment, apiDetails, doUpload, mode, outputDir, memoryFlag, addSnippetsFlag, threadsFlag, config, logger) {
    ...
    if (cacheKey) {
        try {
            ...
            logger.error(`Non-fatal error: Could not restore the cache for ${cacheKey}: ${(_a = e) === null || _a === void 0 ? void 0 : _a.message}`);
        }
    }
    ...
    if (cacheHit) {
        ...
        queriesStats = await runQueries(outputDir, memoryFlag, addSnippetsFlag, threadsFlag, config, logger);
    }
    if (!doUpload) {
    ...
    if (cacheKey && !cacheHit) {
        try {
            ...
            logger.error(`Non-fatal error: Could not cache the results for ${cacheKey}: ${(_b = e) === null || _b === void 0 ? void 0 : _b.message}`);
        }
    }
